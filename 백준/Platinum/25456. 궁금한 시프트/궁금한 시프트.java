...
public class Main {
	...
	public static void main(String[] args) throws Exception {
		...
		long[] arr1 = new long[flen], arr2 = new long[flen], conv = new long[flen];
		for(int i=0;i<s1.length;i++) arr1[s1.length-1-i]=s1[i]-'0';
		for(int i=0;i<s2.length;i++) arr2[s2.length-1-i]=s2[i]-'0';
		arr1 = fft(arr1,false);
		arr2 = fft(arr2,false);
		for(int i=0;i<flen;i++) conv[i] = (arr1[i]*arr2[i])%MOD;
		conv = fft(conv,true);
		for(int i=0;i<flen;i++) answer = Math.max(answer,conv[i]);
