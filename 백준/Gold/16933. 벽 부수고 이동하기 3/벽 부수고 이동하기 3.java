...
public class Main{
    ...
    public static void main(String[] args) throws Exception{
        ...
        map = new char[R][C]; visit = new boolean[R][C][K+1][2];
        for(int i=0;i<R;i++){
        ...
        while(!q.isEmpty()){
            ...
            for (int[] d : delta) {
                ...
                if (isIn(nr, nc)) {
                    if (map[nr][nc] == '0' && !visit[nr][nc][status.wall][status.time%2]) {
                        visit[nr][nc][status.wall][status.time%2] = true;
                        q.add(new Status(nr, nc, status.time + 1, status.wall));
                    } else if (map[nr][nc] == '1' && status.time%2==1 && status.wall!=0 && !visit[nr][nc][status.wall-1][status.time%2]) {
                        visit[nr][nc][status.wall-1][status.time%2] = true;
                        q.add(new Status(nr, nc, status.time + 1, status.wall - 1));
                    }
                }
            }
            if (!visit[status.r][status.c][status.wall][status.time%2]) {
            	visit[status.r][status.c][status.wall][status.time%2] = true;
            	status.time++;
            	q.add(status);
            }
        }
