...
public class Main {
    ...
    static StringBuilder sb = new StringBuilder(), blank = new StringBuilder();
    ...
    public static void main(String[] args) throws Exception {
        ...
        for(int i=1, size = 1;i<=N;i++, size*=5){
            next.clear();
            blank.setLength(0);
            for(int j=0;j<size;j++) blank.append(' ');
            for(int r=0;r<5;r++){
                for(int j=0;j<size;j++) {
                    sb.setLength(0);
                    for (int c = 0; c < 5; c++) sb.append(star[r][c]?now.get(j):blank.toString());
